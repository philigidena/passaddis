# ECS Fargate Module for PassAddis Backend
...
resource "aws_ecs_cluster_capacity_providers" "main" {
  ...
  default_capacity_provider_strategy {
    capacity_provider = "FARGATE_SPOT"  # Use spot instances to save 70%
    weight            = 1
  }
}
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:s3:::${var.name_prefix}-uploads/*"
      }
    ]
  })
}
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([
    {
      name      = "api"
      image     = var.container_image != "" ? var.container_image : "amazon/amazon-ecs-sample"
      essential = true
...
resource "aws_ecs_service" "api" {
  ...
  desired_count   = 1  # Start with 1 task to minimize costs
  launch_type     = "FARGATE"
