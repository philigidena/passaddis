    ...
    <DashboardLayout title="Admin Panel" navItems={navItems} accentColor="primary">
      ...
      {stats?.shop && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
          <StatCard
            title="Shop Owners"
            value={stats.shop.owners.total}
            subtitle={`${stats.shop.owners.pending} pending approval`}
            icon={<StoreIcon />}
            color="yellow"
          />
          <StatCard
            title="Shop Items"
            value={stats.shop.items.total}
            subtitle={`${stats.shop.items.inStock} in stock`}
            icon={<ShopIcon />}
            color="green"
          />
          <StatCard
            title="Shop Orders"
            value={stats.shop.orders.total}
            subtitle={`${stats.shop.orders.thisMonth.total} this month`}
            icon={
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <StatCard
            title="Shop Revenue"
            value={formatCurrency(stats.shop.orders.revenue)}
            subtitle={`${formatCurrency(stats.shop.orders.thisMonth.revenue)} this month`}
            icon={
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <button
          onClick={() => navigate('/admin/shop-owners?status=PENDING')}
          className="flex flex-col items-center justify-center gap-2 p-4 rounded-xl bg-gray-800 border border-gray-700 hover:border-primary/50 transition-colors"
        >
          <div className="p-3 rounded-lg bg-orange-500/20 text-orange-400">
            <StoreIcon />
          </div>
          <span className="text-sm text-gray-300">Approve Shop Owners</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-sm text-gray-300">Promo Codes</span>
        </button>
      </div>
    </DashboardLayout>
  );
